...
console.log(chalk.bold.overline.underline("\n\t\t|🎉 Welcome to the ATM 🎉|\n\t\t"));
...
if (!isNaN(mypin)) {
    // Verify PIN
    console.log(chalk.italic.bold("Your PIN code has been successfully verified.✅\n"));
    // Prompt user to select an option
    let optionAns = await inquirer.prompt([
        {
            name: "option",
            message: chalk.italic.bold("Please indicate your preference by selecting an option.\n"),
            type: "list",
            choices: [
                "Check Balance",
                "Withdraw",
    ...
    if (optionAns.option === "Withdraw") {
        let amountAns = await inquirer.prompt([
            {
                name: "amount",
                type: "number",
                message: chalk.bold.italic("Please specify the amount you wish to withdraw: "),
                transformer: (input) => {
                    return chalk.italic.cyan.bold(input);
                },
            },
        ...
        if (amountAns.amount > myBalance) {
            ...
            console.log(chalk.bold.italic("Your account balance is insufficient for this transaction") + chalk.red.bold("!"));
        }
        //= , -= , +=  ==> assignment operator.
        else {
            myBalance -= amountAns.amount;
            // Update balance after withdrawal
            console.log(chalk.bold.italic(`Here is your remaining balance💰 : `) +
        ...
        console.log(chalk.bold.italic(`Your current balance is:`) +
            chalk.cyan.bold.italic(`${myBalance}\n`));
    // Fast Cash
    ...
    if (optionAns.option === "Change Pin") {
        ...
        console.log(chalk.bold.italic(`Your new PIN is: `) +
    ...
    if (optionAns.option === "Add Funds") {
        let fundsToAdd = await inquirer.prompt([
            {
                name: "amount",
                type: "number",
                message: chalk.bold.italic("Please specify the amount you wish to add to your balance: "),
                transformer: (input) => {
                    return chalk.italic.cyan.bold(input);
                },
            },
        ...
        console.log(chalk.bold.italic(`Funds added successfully. Your new balance is: `) +
            chalk.cyan.bold.italic(`${myBalance}\n`));
    }
    // Funds Transfer
    if (optionAns.option === "Funds Transfer") {
        let fundsToSend = await inquirer.prompt([
            {
                ...
                message: chalk.bold.italic("Please enter the recipient's account number: "),
                type: "number",
                transformer: (input) => {
                    return chalk.italic.cyan.bold(input);
                },
            },
            {
                name: "amount",
                type: "number",
                message: chalk.bold.italic("Please specify the amount you wish to send: "),
                transformer: (input) => {
                    return chalk.italic.cyan.bold(input);
                },
            },
        ...
        if (fundsToSend.amount > myBalance) {
            ...
            console.log(chalk.bold.italic(`Funds sent successfully. Your remaining balance is: `) + chalk.cyan.bold.italic(`${myBalance}\n`));
        }
    }
    // Exit
    if (optionAns.option === "Exit") {
        console.log(chalk.bold.yellow.italic("Thank you for using the ATM. Goodbye!\n"));
    }
}
// Incorrect PIN
else {
